...
export function lengthOfLongestSubstringTwo(s: string): number {
	if (s.length <= 1) return s.length;
...
export function lengthOfLongestSubarrOne(arr: any[]): number {
...
export function lengthOfLongestSubarrTwo(arr: any[]): number {
	if (arr.length <= 1) return arr.length;
