 ...
 */
export function hasPathSumOne(root: TreeNode | null, targetSum: number): boolean {
	if (root == null) return false;
	if (root.left == null && root.right == null && root.val === targetSum) return true;
	return (
		hasPathSumOne(root.left, targetSum - root.val) ||
		hasPathSumOne(root.right, targetSum - root.val)
	);
}
 ...
 * 空间复杂度：O(n)，最坏情况二叉树化为链表，递归栈空间最大为n
 */
export function hasPathSumTwo(root: TreeNode | null, targetSum: number): boolean {
	if (root == null) return false;
